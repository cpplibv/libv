...
SUITE(VecTests) {
	...
	TEST(unionValue) {
		...
		CHECK_EQUAL(42, vec.x);
		CHECK_EQUAL(42, vec.r);
		CHECK_EQUAL(42, vec.s);
		CHECK_EQUAL(42, vec.u);
		CHECK_EQUAL(42, vec.ptr[0]);
		CHECK_EQUAL(1, vec.y);
		CHECK_EQUAL(1, vec.g);
		CHECK_EQUAL(1, vec.t);
		CHECK_EQUAL(1, vec.v);
		CHECK_EQUAL(1, vec.ptr[1]);
		CHECK_EQUAL(2, vec.z);
		CHECK_EQUAL(2, vec.b);
		CHECK_EQUAL(2, vec.p);
		CHECK_EQUAL(2, vec.ptr[2]);
		CHECK_EQUAL(3, vec.w);
		CHECK_EQUAL(3, vec.a);
		CHECK_EQUAL(3, vec.q);
		CHECK_EQUAL(3, vec.ptr[3]);

		CHECK_EQUAL(4 * sizeof (float), sizeof (vec));
	}

	TEST(unionAddress) {
		vec4_t<double> vec(0, 1, 2, 3);

		CHECK_EQUAL(&vec.x, vec.ptr);
		CHECK_EQUAL(&vec.y, vec.ptr + 1);
		CHECK_EQUAL(&vec.z, vec.ptr + 2);
		CHECK_EQUAL(&vec.w, vec.ptr + 3);

		CHECK_EQUAL(&vec.x, &vec.ptr[0]);
		CHECK_EQUAL(&vec.x, &vec.r);
		CHECK_EQUAL(&vec.x, &vec.s);
		CHECK_EQUAL(&vec.x, &vec.u);

		CHECK_EQUAL(&vec.y, &vec.ptr[1]);
		CHECK_EQUAL(&vec.y, &vec.g);
		CHECK_EQUAL(&vec.y, &vec.t);
		CHECK_EQUAL(&vec.y, &vec.v);

		CHECK_EQUAL(&vec.z, &vec.ptr[2]);
		CHECK_EQUAL(&vec.z, &vec.b);
		CHECK_EQUAL(&vec.z, &vec.p);

		CHECK_EQUAL(&vec.w, &vec.ptr[3]);
		CHECK_EQUAL(&vec.w, &vec.a);
		CHECK_EQUAL(&vec.w, &vec.q);

		CHECK_EQUAL(4 * sizeof (double), sizeof (vec));
	}
	...
	TEST(normalize) {
		...
		CHECK_CLOSE(1, vec0.normalized().length(), 0.0001);
	}
}