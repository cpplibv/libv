// Project: libv.re, File: test/libv/re/test_glsl_compose.cpp
...
TEST_CASE("GLSL source include pre-processing", "[libv.re.glsl_compose]") {
	const auto includer = [](const auto& str, const std::vector<std::pair<std::string, int>>&) -> std::string {
		if (str == "A.glsl")
		...
		if (str == "EinE.glsl")
			return "#include <EinE.glsl>\nE2\nE3\nE4\n";

		if (str == "HinGinH.glsl")
			return "#include <GinHinG.glsl>\nH2";
		if (str == "GinHinG.glsl")
			return "#include <HinGinH.glsl>\nG2";

		if (str == "P.glsl")
			return "#pragma once\nP2";
		if (str == "Q.glsl")
			return "#include <P.glsl>\nQ2";
		...
		return "ERROR Could not find source \"" + std::string(str) + '"';
	};

	const auto test = [&] (auto source) {
		return libv::re::glsl_compose_from_source(includer, source).code;
	};
