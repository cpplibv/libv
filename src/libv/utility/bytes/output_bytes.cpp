...
output_bytes::output_bytes(std::byte* data, std::size_t size) noexcept :
	object(data),
	output_offset(0),
	output_size(size),
	write_fn(+[](void* object_, const std::byte* ptr, std::size_t pos, std::size_t size_) noexcept {
...
output_bytes::output_bytes(std::ostream& s) noexcept {
	...
	output_offset = cur_p < 0 ? 0 : static_cast<std::size_t>(cur_p);

	output_size = std::numeric_limits<std::size_t>::max();

	write_fn = +[](void* object, const std::byte* ptr, std::size_t pos, std::size_t size) noexcept {
