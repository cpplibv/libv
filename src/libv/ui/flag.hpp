...
static constexpr Flag_t parentDependOnLayout  = libv::bit(21); /// Component dynamic content size change will affect parent's layout (if not set, parent layout does not have to be invalidated on content's size change)
