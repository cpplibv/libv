...
enum class Orientation2 : int8_t {
	down_left = 0,
	down_right = 1,
	left_down = 2,
	left_up = 3,
...
public:
	explicit(false) constexpr inline Orientation2_type(enum_type value) noexcept :
		enum_value_(value) {
		assert(static_cast<underlying_type>(enum_value_) >= 0 && static_cast<underlying_type>(enum_value_) < 8);
	}
...
public:
	[[nodiscard]] constexpr inline enum_type next() noexcept {
		...
		return static_cast<enum_type>(u == 7 ? 0 : u + 1);
	}
	[[nodiscard]] constexpr inline enum_type prev() noexcept {
		const underlying_type u = static_cast<underlying_type>(enum_value_);
		return static_cast<enum_type>(u == 0 ? 7 : u - 1);
	}
...
[[nodiscard]] constexpr inline int8_t underlying(Orientation2 enum_value) noexcept {
	return static_cast<int8_t>(enum_value);
}
[[nodiscard]] constexpr inline int8_t operator+(Orientation2 enum_value) noexcept {
	return static_cast<int8_t>(enum_value);
}
