// Project: libv.ui, File: src/libv/ui/component_system/flag.hpp
...
class tag_Flag;
using Flag_t = libv::flag_enum<uint32_t, tag_Flag>;

namespace Flag {

static constexpr Flag_t none = Flag_t{};

static constexpr Flag_t render                 = libv::bit(0);
static constexpr Flag_t layout                 = libv::bit(1); /// Primarily used to exclude components that are marked for removal from the current loop's layout (This flag could be folded under pendingDetachSelf)
static constexpr Flag_t disabled               = libv::bit(2); /// Component is disabled (No interaction to inputs or focus)
static constexpr Flag_t disableControlled      = libv::bit(3); /// This component controls the disable directive of itself and its subtree
static constexpr Flag_t signal                 = libv::bit(4); /// Component has or had at least one slot connected. Once set this flag is never resets
static constexpr Flag_t slot                   = libv::bit(5); /// Component has or had at least one signal connected. Once set this flag is never resets
static constexpr Flag_t focused                = libv::bit(6);
//static constexpr Flag_t ____                   = libv::bit(7);
...
static constexpr Flag_t updatedSize            = libv::bit(21); /// Component's size was changed since it last render
static constexpr Flag_t updatedPosition        = libv::bit(22); /// Component's position was changed since it last render
static constexpr Flag_t layoutDependsOnContent = libv::bit(23); /// Component dynamic content size change will affect parent's layout (if not set, parent layout does not have to be invalidated on content's size change)

static constexpr Flag_t focusableChild         = libv::bit(24);
...
constexpr inline Flag_t calculatePropagateFlags(Flag_t flags) noexcept {
	const auto mask_propagatableSelf = Flag_t{Flag::mask_propagate.value() << 1u};
