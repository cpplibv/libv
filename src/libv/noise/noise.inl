...
template <typename T>
[[nodiscard]] constexpr inline T noise_simplex_v(Seed seed, T x, T y) noexcept {
	using SIMD = SIMD_DISPATCH<T>;
	...
	auto f = F2 * (x + y);
	auto x0 = SIMD::floor(x + f);
	auto y0 = SIMD::floor(y + f);

	const auto i = SIMD::convertf32_i32(x0) * SIMD::v_i32(Primes::X);
	const auto j = SIMD::convertf32_i32(y0) * SIMD::v_i32(Primes::Y);

	T g = T(G2) * (x0 + y0);
	x0 = x - (x0 - g);
	y0 = y - (y0 - g);

	const auto i1 = SIMD::gt(x0, y0);
	//mask32v j1 = ~i1; //NMasked funcs

	T x1 = SIMD::masked_sub_f32(x0, T(1.f), i1) + T(G2);
	T y1 = SIMD::n_masked_sub_f32(y0, T(1.f), i1) + T(G2);

	T x2 = x0 + T(G2 * 2 - 1);
	T y2 = y0 + T(G2 * 2 - 1);

//	T t0 = -(x0 * x0) + (-(y0 * y0)) + 0.5f;
//	T t1 = -(x1 * x1) + (-(y1 * y1)) + 0.5f;
//	T t2 = -(x2 * x2) + (-(y2 * y2)) + 0.5f;
	T t0 = SIMD::n_mul_add(x0, x0, SIMD::n_mul_add(y0, y0, typename SIMD::float32v(0.5f)));
	T t1 = SIMD::n_mul_add(x1, x1, SIMD::n_mul_add(y1, y1, typename SIMD::float32v(0.5f)));
	T t2 = SIMD::n_mul_add(x2, x2, SIMD::n_mul_add(y2, y2, typename SIMD::float32v(0.5f)));
	...
	T n0 = SIMD::gradientDot(SIMD::hashPrimes(seed, i, j), x0, y0);
	T n1 = SIMD::gradientDot(SIMD::hashPrimes(seed, SIMD::masked_add_i32(i, SIMD::v_i32(Primes::X), i1), SIMD::n_masked_add_i32(j, SIMD::v_i32(Primes::Y), i1)), x1, y1);
	T n2 = SIMD::gradientDot(SIMD::hashPrimes(seed, i + SIMD::v_i32(Primes::X), j + SIMD::v_i32(Primes::Y)), x2, y2);

	return T(38.283687591552734375f) * SIMD::mul_add(n0, t0, SIMD::mul_add(n1, t1, n2 * t2));
}
...
///		The implementation is optimized for glsl and therefore around 2x slower on CPU
template <typename T>
...
constexpr inline libv::vec2f singleWarpSimplexGradient(Seed seed, float x, float y, float warpAmp, float frequency, bool outGradOnly) {
	const float SQRT3 = 1.7320508075688772935274463415059f;
	const float G2 = (3 - SQRT3) / 6;
...
constexpr inline libv::vec2f doSingleWarp(Seed seed, float x, float y, WarpType type, float amp, float freq) {
	switch (type) {
...
constexpr inline libv::vec2f transformWarpCoordinate(float x, float y, WarpType type) {
...
constexpr inline libv::vec2f warpFractalIndependent(Seed seed, float x, float y,
...
constexpr inline libv::vec2f warp(Seed seed, float x, float y, WarpFractalType type,
