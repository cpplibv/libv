...
void CameraControl::register_controls(libv::ctrl::FeatureRegister controls) {
	...
	controls.feature_analog<BaseCameraOrbit>(fname_camera_orbit_roll, sg_rotate, [](const auto& arg, BaseCameraOrbit& ctx) {
		const auto value = arg.value_f();
		ctx.orbit_roll(value);
	});

	controls.feature_analog<BaseCameraOrbit>(fname_camera_orbit_pitch, sg_rotate, [](const auto& arg, BaseCameraOrbit& ctx) {
		const auto value = arg.value_f();
		ctx.orbit_pitch(value);
	});

	controls.feature_analog<BaseCameraOrbit>(fname_camera_orbit_yaw, sg_rotate, [](const auto& arg, BaseCameraOrbit& ctx) {
		const auto value = arg.value_f();
		ctx.orbit_yaw(value);
	});


	controls.feature_analog<BaseCameraOrbit>(fname_camera_move_forward, sg_translate, [](const auto& arg, BaseCameraOrbit& ctx) {
		const auto value = arg.value_f() * std::pow(ctx.orbit_distance(), c0_translate);
		ctx.move_forward(value);
	});

	controls.feature_analog<BaseCameraOrbit>(fname_camera_move_right, sg_translate, [](const auto& arg, BaseCameraOrbit& ctx) {
		const auto value = arg.value_f() * std::pow(ctx.orbit_distance(), c0_translate);
		ctx.move_right(value);
	});

	controls.feature_analog<BaseCameraOrbit>(fname_camera_move_up, sg_translate, [](const auto& arg, BaseCameraOrbit& ctx) {
		const auto value = arg.value_f() * std::pow(ctx.orbit_distance(), c0_translate);
		ctx.move_up(value);
	});


	controls.feature_analog<BaseCameraOrbit>(fname_camera_orbit_distance, sg_orbit, [](const auto& arg, BaseCameraOrbit& ctx) {
	...
	controls.feature_action<BaseCameraOrbit>(fname_camera_snap_angle, [](const auto&, BaseCameraOrbit& camera) {
	...
	controls.feature_action<BaseCameraOrbit>(fname_camera_snap_position, [](const auto&, BaseCameraOrbit& camera) {
	...
	controls.feature_action<BaseCameraOrbit>(fname_camera_debug, [](const auto&, BaseCameraOrbit& camera) {
		log_sun.info("Camera:"
					   "\n\tEye     {}"
					   "\n\tForward {}"
					   "\n\tRight   {}"
...
void CameraControl::bind_default_controls(libv::ctrl::BindingRegister controls) {
	controls.bind(fname_camera_orbit_distance, "T", -1);
	controls.bind(fname_camera_orbit_distance, "G");
	controls.bind(fname_camera_orbit_distance, "Scroll", -1);
	controls.bind(fname_camera_orbit_distance, "LMB + RMB + Mouse Y", -1);
	controls.bind(fname_camera_orbit_distance, "LMB + RMB + Mouse X", 0);
	...
	controls.bind(fname_camera_orbit_pitch, "W", -1);
	controls.bind(fname_camera_orbit_pitch, "S");
	controls.bind(fname_camera_orbit_pitch, "LMB + Mouse Y");

	controls.bind(fname_camera_orbit_roll, "Q", -1);
	controls.bind(fname_camera_orbit_roll, "E");

	controls.bind(fname_camera_orbit_yaw, "A");
	controls.bind(fname_camera_orbit_yaw, "D", -1);
	controls.bind(fname_camera_orbit_yaw, "LMB + Mouse X");

	controls.bind(fname_camera_move_forward, "Up");
	controls.bind(fname_camera_move_forward, "Down", -1);
	controls.bind(fname_camera_move_forward, "RMB + Mouse Y", -1);

	controls.bind(fname_camera_move_right, "Right");
	controls.bind(fname_camera_move_right, "Left", -1);
	controls.bind(fname_camera_move_right, "RMB + Mouse X", -1);

	controls.bind(fname_camera_move_up, "Y");
	controls.bind(fname_camera_move_up, "H", -1);
	controls.bind(fname_camera_move_up, "MMB + Mouse Y");

	// Debug / dev tools:
	controls.bind(fname_camera_debug, "Shift + Ctrl + P");
	controls.bind(fname_camera_snap_position, "Shift + Ctrl + O");
	controls.bind(fname_camera_snap_angle, "Shift + Ctrl + I");

	controls.bind(fname_camera_snap_angle, "Shift + Ctrl + U");
	controls.bind(fname_camera_snap_position, "Shift + Ctrl + U");
}
...
void CameraControl2D::register_controls(libv::ctrl::FeatureRegister controls) {
	...
	controls.feature_analog<CameraOrtho>(fname_camera_zoom, sg_zoom, [](const auto& arg, CameraOrtho& ctx) {
...
//		ctx.zoom(std::clamp(value, 0.000001f, 1000.f));
	...
	controls.feature_analog<CameraOrtho>(fname_camera_move_forward, sg_translate, [](const auto& arg, CameraOrtho& ctx) {
		const auto value = arg.value_f() * std::pow(ctx.zoom() * 250.f, c0_translate);
		ctx.move_forward(value);
	});

	controls.feature_analog<CameraOrtho>(fname_camera_move_right, sg_translate, [](const auto& arg, CameraOrtho& ctx) {
		const auto value = arg.value_f() * std::pow(ctx.zoom() * 250.f, c0_translate);
		ctx.move_right(value);
	});
}
...
void CameraControl2D::bind_default_controls(libv::ctrl::BindingRegister controls) {
	controls.bind(fname_camera_zoom, "Scroll", -1);
	controls.bind(fname_camera_zoom, "T");
	controls.bind(fname_camera_zoom, "G", -1);
