...
void increase_ref_count(MeshCube* ptr) {
	libv::ref_count_access::increase_ref_count(ptr);
}
void decrease_ref_count(MeshCube* ptr) {
...
BoundingSphere MeshCube::boundingSphere() const {
	return {libv::vec3f{0, 0, 0}, std::sqrt(3.f) * 0.5f};
}
