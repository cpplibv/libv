...
validation_result Sequence::validate() const noexcept {
	...
	// stateful_input_in_non_last_step

	for (size_t i = 0; i < combinations.size(); ++i) {
		const auto is_last_combination = i + 1 == combinations.size();

		if (is_last_combination)
			break;

		for (const auto& input : combinations[i].inputs)
			if (input.dia == DigitalInputAction::hold || input.dia == DigitalInputAction::free)
				result |= validation_result::stateful_input_in_non_last_step;
	}

	// analog_input_in_non_last_step
	...
	for (size_t i = 0; i < combinations.size(); ++i) {
		...
		for (const auto& input : combinations[i].inputs)
			if (input.id.is_analog())
				result |= validation_result::analog_input_in_non_last_step;
	}
