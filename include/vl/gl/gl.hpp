...
inline void enableVertexAttribArray(const vl::gl::Attribute& attribute) {
	glEnableVertexAttribArray(static_cast<GLuint>(attribute));
}

// glVertexAttribPointer ---------------------------------------------------------------------------
inline void vertexAttribPointer(
		const vl::gl::Attribute& attribute,
		GLint size,
		const vl::gl::VertexAttribType& type,
...
inline void vertexAttribLPointer(
	...
	glVertexAttribLPointer(
			static_cast<GLuint> (attribute),
			size,
			static_cast<GLenum> (vl::gl::VertexAttribType::DOUBLE),
			stride,
			pointer);
}
