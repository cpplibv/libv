...
//	controls.feature_analog<Camera3>("camera.roll", sg_rotate, [](const auto& arg, Camera3& ctx) {
//		ctx.roll(arg.value_f());
//	});
//
//	controls.feature_analog<Camera3>("camera.pitch", sg_rotate, [](const auto& arg, Camera3& ctx) {
//		ctx.pitch(arg.value_f());
//	});
//
//	controls.feature_analog<Camera3>("camera.yaw", sg_rotate, [](const auto& arg, Camera3& ctx) {
//		ctx.yaw(arg.value_f());
//	});


	controls.feature_analog<Camera3>("camera.rotate_x", sg_rotate, [](const auto& arg, Camera3& ctx) {
		const auto value = arg.value_f();
		ctx.rotate_x(value);
	});

	controls.feature_analog<Camera3>("camera.rotate_y", sg_rotate, [](const auto& arg, Camera3& ctx) {
		const auto value = arg.value_f();
		ctx.rotate_y(value);
	});

	controls.feature_analog<Camera3>("camera.rotate_z", sg_rotate, [](const auto& arg, Camera3& ctx) {
		const auto value = arg.value_f();
		ctx.rotate_z(value);
	});


	controls.feature_analog<Camera3>("camera.translate_forward", sg_translate, [](const auto& arg, Camera3& ctx) {
		const auto value = arg.value_f() * std::pow(ctx.orbit(), c0_translate);
		ctx.translate_forward(value);
	});

	controls.feature_analog<Camera3>("camera.translate_right", sg_translate, [](const auto& arg, Camera3& ctx) {
		const auto value = arg.value_f() * std::pow(ctx.orbit(), c0_translate);
		ctx.translate_right(value);
	});

	controls.feature_analog<Camera3>("camera.translate_up", sg_translate, [](const auto& arg, Camera3& ctx) {
		const auto value = arg.value_f() * std::pow(ctx.orbit(), c0_translate);
		ctx.translate_up(value);
	});


	controls.feature_analog<Camera3>("camera.pivot_orbit", sg_orbit, [](const auto& arg, Camera3& ctx) {
...
void CameraBehaviour::bind_controls(libv::ctrl::Controls& controls) {
	...
	controls.bind("camera.translate_forward", "Up", -1);
	controls.bind("camera.translate_forward", "Down");
	controls.bind("camera.translate_forward", "RMB + Mouse Y");

	controls.bind("camera.translate_right", "Left", -1);
	controls.bind("camera.translate_right", "Right");
	controls.bind("camera.translate_right", "RMB + Mouse X", -1);

	controls.bind("camera.translate_up", "Y");
	controls.bind("camera.translate_up", "H", -1);
}
