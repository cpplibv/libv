...
//	orbit_yaw
	...
	void translate_forward(float value) {
		position_.y -= value * std::cos(rotation_.z);
		position_.x -= value * std::sin(rotation_.z);
	}
	void translate_right(float x) {
		position_.y += x * std::cos(rotation_.z + libv::pi / 2.0f);
		position_.x += x * std::sin(rotation_.z + libv::pi / 2.0f);
	}
	void translate_up(float x) {
		position_.z += x;
	}
