// Project: libv, File: app/theme/image.hpp
...
public:
	[[nodiscard]] inline libv::vec4f& color(std::size_t x, std::size_t y) noexcept {
		return color_0(x, y);
	}

	[[nodiscard]] inline const libv::vec4f& color(std::size_t x, std::size_t y) const noexcept {
		return color_0(x, y);
	}

	[[nodiscard]] inline float& sdistance(std::size_t x, std::size_t y) noexcept {
		return sdistance_field(x, y);
	}

	[[nodiscard]] inline const float& sdistance(std::size_t x, std::size_t y) const noexcept {
		return sdistance_field(x, y);
	}
