...
//	});
	nexus.connect<RequestClientExit>(this, [this](const RequestClientExit&) {
		frame.closeDefault();
	});
}
...
//void GameClient::execute() {
//	frame.show();
//	frame.join();
//}
//
//void GameClient::enterSinglePlayer() {
//	scene_main.openSP();
//}
//
//void GameClient::enterMultiPlayerClient(std::string server_address, uint16_t server_port) {
//	scene_main.openMPClient(std::move(server_address), server_port);
//}
//
//void GameClient::enterMultiPlayerServer(uint16_t port) {
//	scene_main.openMPServer(port);
//}

void GameClient::init_ui() {
	ui.add(createSceneMainMenu(nexus));
}
